...
local PI_180 = PI/180
local DISTANCE = 5
function asin(x) return math.sin(x*PI_180) end
function acos(x) return math.cos(x*PI_180) end

local function spawn_for_player_model(player, model)
    local c = ENTITY.GET_ENTITY_COORDS(PLAYER.PLAYER_PED_ID(), 1)
...
function giftvehicle(pid ,pedid)
    local pveh = PED.GET_VEHICLE_PED_IS_IN(pedid, 1)
	
        local veh = spawn_for_player_model(pid, ENTITY.GET_ENTITY_MODEL(pveh))
        local phash = NETWORK.NETWORK_HASH_FROM_PLAYER_HANDLE(pid)
...
local Gift = Silent:add_tab("Gift Vehicle")
Gift:add_button("Gift Vehicle",
function ()
		local pid = PLAYER.PLAYER_ID()
		local pedid = PLAYER.GET_PLAYER_PED_ID(pid)
		giftvehicle(pid, pedid)
end)
