...
--Agency--
	...
	Agency:add_array_item("VIP Contract", AgencyContracts, function() return a1 end, 
		...
		elseif VIPCon == 3 then stats.set_int(MPX .. "FIXER_STORY_BS", 4)
		elseif VIPCon == 4 then stats.set_int(MPX .. "FIXER_STORY_BS", 12)
		elseif VIPCon == 5 then stats.set_int(MPX .. "FIXER_STORY_BS", 28)
		elseif VIPCon == 6 then stats.set_int(MPX .. "FIXER_STORY_BS", 60)
		elseif VIPCon == 7 then stats.set_int(MPX .. "FIXER_STORY_BS", 124)
		elseif VIPCon == 8 then stats.set_int(MPX .. "FIXER_STORY_BS", 252)
		elseif VIPCon == 9 then stats.set_int(MPX .. "FIXER_STORY_BS", 508)
		elseif VIPCon == 10 then stats.set_int(MPX .. "FIXER_STORY_BS", 2044)
		elseif VIPCon == 11 then stats.set_int(MPX .. "FIXER_STORY_BS", 4095)
		elseif VIPCon == 12 then stats.set_int(MPX .. "FIXER_STORY_BS", -1) stats.set_int(MPX .. "FIXER_STORY_STRAND", -1) end 
		a1 = VIPCon end)	
...
--Auto Shop--
	...
	AutoShop:add_array_item("Auto Shop Mission", {"Select", "Union Depository", "Superdollar Deal", "Bank Contract", "ECU Job", "Prison Contract", "Agency Deal", "Lost Contract", "Data Contract"}, function() return a3 end, 
		...
		elseif AutShoMis == 4 then stats.set_int(MPX .. "TUNER_CURRENT", 2)
		elseif AutShoMis == 5 then stats.set_int(MPX .. "TUNER_CURRENT", 3)
		elseif AutShoMis == 6 then stats.set_int(MPX .. "TUNER_CURRENT", 4)
		elseif AutShoMis == 7 then stats.set_int(MPX .. "TUNER_CURRENT", 5)
		elseif AutShoMis == 8 then stats.set_int(MPX .. "TUNER_CURRENT", 6)
		elseif AutShoMis == 9 then stats.set_int(MPX .. "TUNER_CURRENT", 7) end
		a3 = AutShoMis end)
...
--Cayo Perico--
	...
	CPP:add_array_item("Primary Target", {"Select", "Tequila (900k)", "Necklace (1m)", "Bonds (1,1m)", "Diamond (1,3m)", "Statue (1,9m)"}, function() return a6 end, 
		...
		a6 = PriTar end)
			...
			a7 = 1
	CPP:add_array_item("Fill Compound Storages", {"None", "Cash", "Weed", "Coke", "Gold"}, function() return a7 end, 
		...
		elseif FilComSto == 2 then stats.set_int(MPX .. "H4LOOT_CASH_C", 255)
		...
		a7 = FilComSto end)
			...
			a8 = 1
	CPP:add_array_item("Fill Island Storages", {"None", "Cash", "Weed", "Coke", "Gold"}, function() return a8 end, 
		...
		elseif FilIslSto == 2 then stats.set_int(MPX .. "H4LOOT_CASH_I", 16777215)
...
--Cash Remover--
	...
	CashRemover:add_bare_item("", 
		function() 
			...
			if CashToRemove == 0 then CashToRemove = "0" end
		return "Remove Cash: $" ..  FormatMoney(CashToRemove) end, 
		function() globals.set_int(2764426 + 36, CashToRemove) 
				   sleep(1)
			       DefNum1Cur = 1
				   DefNum2Cur = 1
				   DefNum3Cur = 1
...
--Rank--
	...
	Rank:add_bare_item("", 
		function()
			...
			if CurrentRank == 1 then CurrentRank = " " 
			end
		return "Current Rank: " .. CurrentRank 
			...
			DefNum29Cur = 1
	Rank:add_array_item("Char #1", NumberList2, function() return DefNum29Cur end, function(NewNum29)
		DefNum29 = NumberList2[NewNum29]
	    DefNum29Cur = NewNum29 end)
		
			DefNum30 = NumberList1[1]
			DefNum30Cur = 1
	Rank:add_array_item("Char #2", NumberList1, function() return DefNum30Cur end, function(NewNum30)
		DefNum30 = NumberList1[NewNum30]
		DefNum30Cur = NewNum30 end)
		
			DefNum31 = NumberList1[1]
			DefNum31Cur = 1
	Rank:add_array_item("Char #3", NumberList1, function() return DefNum31Cur end, function(NewNum31)
		DefNum31 = NumberList1[NewNum31]
		DefNum31Cur = NewNum31 end)
		
			DefNum32 = NumberList1[1]
			DefNum32Cur = 1
	Rank:add_array_item("Char #4", NumberList1, function() return DefNum32Cur end, function(NewNum32)
		DefNum32 = NumberList1[NewNum32]
		DefNum32Cur = NewNum32 end)
			
	Rank:add_bare_item("", 
		function() 
			if DefNum29 == 0 then NewRank = tonumber(DefNum30 .. DefNum31 .. DefNum32)
			else NewRank = tonumber(DefNum29 .. DefNum30 .. DefNum31 .. DefNum32) 
			end
...
--LS Car Meet--
	...
	LSCarMeetNote:add_action("", function() end)
	LSCarMeetNote:add_action("   If you've used LS Tuners awards unlock", function() end)
